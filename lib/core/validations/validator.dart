...
class Validator {
  ...
  static String? validateDiagnosis(String? value) {
    if (value == null || value.isEmpty) return 'Tanı boş olamaz!';
    return null;
  }

  // Hap sayısı validasyonu
  static String? validatePills(String? value) {
    if (value == null || value.isEmpty) return 'Hap sayısı boş olamaz!';
    final pills = int.tryParse(value);
    if (pills == null || pills <= 0) return 'Geçerli bir sayı giriniz!';
    return null;
  }

  // Günlük doz validasyonu: 1..5
  static String? validateDailyDosage(int dosage) {
    if (dosage < 1 || dosage > 5) return 'Günlük doz 1 ile 5 arasında olmalı!';
    return null;
  }

  // Manuel zaman validasyonu
  ...
  int? autoDaysPerWeek, // sadece otomatik mod için
}) {
