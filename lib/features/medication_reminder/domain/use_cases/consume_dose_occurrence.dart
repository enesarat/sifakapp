...
import 'consume_dose.dart';

class ConsumeDoseOccurrence {
  final ConsumeDose consumeDose;
  final DoseLogRepository logs;
  ConsumeDoseOccurrence(this.consumeDose, this.logs);

  Future<Medication> call({required String medId, required DateTime plannedAt}) async {
    final updated = await consumeDose.call(medId);
    final log = DoseLog(
